import { EPOS, AAO_HNSF } from './guidelines_en.js';
...
function generateSymptomsCheckboxes(guideline) {
  ...
  guidelineInfoDiv.textContent = `Guideline: ${guideline.description} (${guideline.name}), Reference: ${guideline.reference}`;
}
...
function displayResult(probability) {
  ...
  if (typeof probability !== 'undefined') {
    if (probability < 33) {
      ...
      comment = 'Low probability of bacterial sinusitis';
    } else if (probability < 66) {
      resultElement.style.color = 'orange';
      comment = 'Medium probability of bacterial sinusitis';
    } else {
      resultElement.style.color = 'red';
      comment = 'High probability of bacterial sinusitis';
    }
